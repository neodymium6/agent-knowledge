...
impl PathAttestation {
    ...
    fn capture_linux(path: &Path, pinned: &File) -> Result<Self, PathAttestationError> {
        ...
        if FilesystemObjectId::from_metadata(&live) != FilesystemObjectId::from_metadata(&pinned) {
            return Err(PathAttestationError::BindingMismatch);
        }
        ...
        let backing = linux_backing_location(&canonical_path, object.map_or(0, |id| id.device))?;
        Ok(Self {
            canonical_path,
            object,
            ancestors,
...
fn linux_backing_location(
    ...
    linux_backing_location_from(&bytes, canonical_path, device)
}
...
fn linux_backing_location_from(
    ...
    for line in mount_info.split(|byte| *byte == b'\n') {
        ...
        {
            selected = Some((specificity, root.join(relative)));
        }
    }
    let Some((_, path)) = selected else {
        return Err(PathAttestationError::InvalidMountInformation);
    };
    Ok(LinuxBackingLocation { device, path })
}
...
31 30 0:42 /data/content/queue /mnt/queue rw - none /data/content/queue rw\n";
        let content = linux_backing_location_from(mount_info, Path::new("/data/content"), device)
            .unwrap_or_else(|error| panic!("content backing path must resolve: {error}"));
        let queue = linux_backing_location_from(mount_info, Path::new("/mnt/queue"), device)
    ...
    fn mount_paths_decode_kernel_octal_escapes() {
        ...
        let backing =
            linux_backing_location_from(mount_info, Path::new("/mnt/fictional root/child"), device)
