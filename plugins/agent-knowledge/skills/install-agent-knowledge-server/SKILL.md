...
5. Provision one dedicated Gateway account. Its primary group is
   ...
   `agent-knowledge-ssh-shell`. Never use the Worker or queue accounts for SSH.
6. Configure OpenSSH public-key-only access. Give every key a root-controlled
   ...
   Disable password, keyboard-interactive, forwarding, PTY, and user startup
   features for the Gateway account. Validate the complete SSH configuration
...
2. Replace every `example.invalid` image through the overlay with these four
   versioned images: `storage-bootstrap`, `worker`, `queue-ingress`, and
   `openssh-gateway`. The standalone `gateway` image is not part of this
   Kubernetes base.
...
- Test one restricted SSH client destination with `recent`; arbitrary shell
  access must fail.
- Confirm a test request reaches `completed`, creates a Git commit, and switches
  the Quartz release atomically.
