...
jq -e '
  ...
  | def resource($kind; $name):
    ...
    and (stateful_set.spec.template.spec.containers
      | map(.image) | sort == [
        "agent-knowledge-openssh-gateway:0.2.0",
        "agent-knowledge-queue-ingress:0.2.0",
        "agent-knowledge-worker:0.2.0"
      ])
    and (stateful_set.spec.template.spec.initContainers
      | all(.[]; .image == "agent-knowledge-storage-bootstrap:0.2.0"))
...
jq -e '
  ...
  | def resource($kind; $name):
    ...
    and (resource("Job"; "seed-agent-knowledge-quartz-v1").spec.template.spec.containers[0]
      .image == "agent-knowledge-kubernetes-e2e-quartz:0.2.0")
    and (resource("Job"; "seed-agent-knowledge-quartz-v1").spec.template.spec.containers[0]
