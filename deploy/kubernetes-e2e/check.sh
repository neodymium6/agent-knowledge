...
jq -e '
  ...
  | def resource($kind; $name):
    ...
    and (stateful_set.spec.template.spec.containers
      | map(.image) | sort == [
        "agent-knowledge-openssh-gateway:0.1.8",
        "agent-knowledge-queue-ingress:0.1.8",
        "agent-knowledge-worker:0.1.8"
      ])
    and (stateful_set.spec.template.spec.initContainers
      | all(.[]; .image == "agent-knowledge-storage-bootstrap:0.1.8"))
...
jq -e '
  ...
  | def resource($kind; $name):
    ...
    and (resource("Job"; "seed-agent-knowledge-quartz-v1").spec.template.spec.containers[0]
      .image == "agent-knowledge-kubernetes-e2e-quartz:0.1.8")
    and (resource("Job"; "seed-agent-knowledge-quartz-v1").spec.template.spec.containers[0]
