...
use agent_knowledge_protocol::StatusRequest;
use agent_knowledge_queue::{FileQueue, PackagePolicy};

use super::{
    ListenSettings, PublishedSocket, QueueIngressListenerError, SOCKET_MODE, listen_until,
};
...
#[test]
fn replaces_a_stale_socket_after_acquiring_the_listener_lock() {
    let root = TestDirectory::create();
    let socket_path = root.path().join("queue-ingress.sock");
    ...
    let published = PublishedSocket::bind(&socket_path)
        .unwrap_or_else(|error| panic!("stale socket must be replaced: {error}"));
    assert!(socket_path.exists());
    drop(published);
    assert!(!socket_path.exists());
}
